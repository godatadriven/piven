...
class TestMetrics:
    ...
    def test_tf_picp(self):
        ...
        picp_out = picp(y_true_prep, y_pred_prep).numpy()
        np.testing.assert_almost_equal(picp_out, 0.84, 2)

    def test_tf_mpiw(self):
        y_pred_prep = tf.convert_to_tensor(
            np.concatenate(
                [self.y_pi_high.copy(), self.y_pi_low.copy(), self.y_weight.copy()],
                axis=1,
        ...
        mpiw_out = mpiw(y_true_prep, y_pred_prep).numpy()
        np.testing.assert_almost_equal(mpiw_out, 0.943, 3)
